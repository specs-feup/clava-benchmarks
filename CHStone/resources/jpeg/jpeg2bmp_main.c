...
void read_markers(unsigned char *buffer)
{
    int unread_marker;
    int sow_SOI;
    ...
    for (;;)
    {
        ...
        if (unread_marker == M_SOI)
        {
            sow_SOI = 1;
        }
        if (unread_marker == M_SOF0)
        {
            get_sof();
        }
        if (unread_marker == M_SOS)
        {
            get_sos();
            return;
        }
        if (unread_marker == M_DHT)
        {
            get_dht();
        }
        if (unread_marker == M_DQT)
        {
            get_dqt();
        }
        if (unread_marker == M_EOI)
        {
            return;
        }
    }
...
void Write4Blocks(int *store1, int *store2, int *store3, int *store4, int *p_out_vpos, int *p_out_hpos, unsigned char *p_out_buf)
{
    ...
    WriteOneBlock(store1, p_out_buf,
                  p_jinfo_image_width, p_jinfo_image_height,
                  voffs, hoffs);
 ...
 * Decode one block
 */
void decode_block(int comp_no, int *out_buf,
                  int *HuffBuff)
{
 ...
 */
int huff_make_dhuff_tb(int *p_xhtbl_bits, int *p_xhtbl_huffval, int p_dhtbl_ml, int *p_dhtbl_maxcode, int *p_dhtbl_mincode, int *p_dhtbl_valptr)
{
 ...
 *
 */
int DecodeHuffman(int *Xhuff_bits, int *Xhuff_huffval, int Dhuff_ml, int *Dhuff_maxcode, int *Dhuff_mincode, int *Dhuff_valptr)
{
    int code, l, p;
...
void DecodeHuffMCU(int *out_buf, int num_cmp)
{
    ...
    s = DecodeHuffman(&p_jinfo_dc_xhuff_tbl_bits[tbl_no][0],
                      &p_jinfo_dc_xhuff_tbl_huffval[tbl_no][0],
    ...
    for (k = 1; k < DCTSIZE2;)
    { /* JPEG Mistake */
        r = DecodeHuffman(&p_jinfo_ac_xhuff_tbl_bits[tbl_no][0],
                          &p_jinfo_ac_xhuff_tbl_huffval[tbl_no][0],
...
void jpeg_init_decompress()
{
    ...
    tmp = huff_make_dhuff_tb(&p_jinfo_dc_xhuff_tbl_bits[0][0],
                             &p_jinfo_dc_xhuff_tbl_huffval[0][0],
    ...
    tmp = huff_make_dhuff_tb(&p_jinfo_dc_xhuff_tbl_bits[1][0],
                             &p_jinfo_dc_xhuff_tbl_huffval[1][0],
    ...
    tmp = huff_make_dhuff_tb(&p_jinfo_ac_xhuff_tbl_bits[0][0],
                             &p_jinfo_ac_xhuff_tbl_huffval[0][0],
    ...
    tmp = huff_make_dhuff_tb(&p_jinfo_ac_xhuff_tbl_bits[1][0],
                             &p_jinfo_ac_xhuff_tbl_huffval[1][0],
