...
void memset_sha(LONG *s, int c, int n, int e)
{
    LONG uc;
    LONG *p;
    int m;
...
void memcpy_sha(LONG *s1, const BYTE *s2, int n)
{
    LONG *p1;
    BYTE *p2;
    LONG tmp;
...
void sha_update(const BYTE *buffer, int count)
{
    ...
    {
        memcpy_sha(sha_info_data, buffer, SHA_BLOCKSIZE);
        sha_transform();
        buffer += SHA_BLOCKSIZE;
        count -= SHA_BLOCKSIZE;
    }
    memcpy_sha(sha_info_data, buffer, count);
}
...
void sha_final()
{
    ...
    if (count > 56)
    {
        memset_sha(sha_info_data, 0, 64 - count, count);
        sha_transform();
        memset_sha(sha_info_data, 0, 56, 0);
    }
    else
    {
        memset_sha(sha_info_data, 0, 56 - count, count);
    }
