...
void pb_AddSubTimer(struct pb_TimerSet *timers, char *label, enum pb_TimerID pb_Category)
{
  ...
  struct pb_SubTimer *subtimer = (struct pb_SubTimer *)malloc(sizeof(struct pb_SubTimer));
  ...
  {
    subtimerlist = (struct pb_SubTimerList *)malloc(sizeof(struct pb_SubTimerList));
...
void pb_PrintTimerSet(struct pb_TimerSet *timers)
{
  ...
  { // exclude NONE and OVRELAP from this format
